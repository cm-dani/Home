...
class linkedIn{
    ...
    public function __construct(string $app_id, string $app_secret, string $callback, string $scopes, bool $ssl = true){
        ...
        $this->scopes =  $scopes;
        $this->csrf = random_int(111111,99999999999); //Cross-site request forgery
        $this->callback = $callback;
        $this->ssl = $ssl;
    }
    
    public function getAuthUrl(){
        $_SESSION['linkedincsrf']  = $this->csrf;//variable SESSION will use meanwhile the user session is online.
    ...
    protected function curl($url, $parameters, $content_type, $post = true){
        ...
        $result = curl_exec($ch);
        return $result; //Access successful
    }
}